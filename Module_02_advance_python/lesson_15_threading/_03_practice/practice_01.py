...
class ThreadPool:
    ...
    def _worker(self):
        # ребята, нашел, в чем был косяк,
        # прошу прощения, что не проверил предварительно.
        if not self.task_queue.empty():  # вот эта проверка и позволяет все остановить когда все выполнено.
